...
Protocol226.modeSpecificMutators = {
  gsp1: 1 << 15,
  gsp2: 1 << 16,
  gsp3: 1 << 17
};

Protocol226.modeSpecificMutatorNames = {
...
Protocol226.prototype.mutatorsFromFlags = function(flags, gameMode) {
	var muts = [];
  ...
  var idx = 0;
  if (gameMode) {
    var gameModeName = (gameMode < Protocol226.gameModes.length) && Protocol226.gameModes[gameMode] || null;
    var gameSpecificMutators = Protocol226.modeSpecificMutatorNames[gameModeName];
    if (gameSpecificMutators) {
    	_.each(_.keys(Protocol226.modeSpecificMutators), function(mutator) {
    	  if (flags & Protocol226.modeSpecificMutators[mutator]) {
          if (idx < gameSpecificMutators.length) {
            muts.push(gameSpecificMutators[idx]);
          }
        }
        idx++;
    	});
...
function Protocol245() { }

//see: src/game/gamemode.h in RE
Protocol245.mutators = {
	ffa           : 1 << 0,
	coop          : 1 << 1,
	instagib      : 1 << 2,
	medieval      : 1 << 3,
...
Protocol245.modeSpecificMutators = {
  gsp1: 1 << 13,
  gsp2: 1 << 14,
  gsp3: 1 << 15
};

Protocol245.modeSpecificMutatorNames = {
  deathmatch: ['gladiator', 'oldschool'],
  'capture-the-flag': ['quick', 'defend', 'protect'],
  'defend-and-control': ['quick', 'king'],
...
Protocol245.prototype.mutatorsFromFlags = function(flags, gameMode) {
	var muts = [];
	_.each(_.keys(Protocol245.mutators), function(mutator) {
	  if (flags & Protocol245.mutators[mutator]) { muts.push(mutator); }
	});
  var idx = 0;
  if (gameMode) {
    var gameModeName = (gameMode < Protocol245.gameModes.length) && Protocol245.gameModes[gameMode] || null;
    var gameSpecificMutators = Protocol245.modeSpecificMutatorNames[gameModeName];
    if (gameSpecificMutators) {
    	_.each(_.keys(Protocol245.modeSpecificMutators), function(mutator) {
    	  if (flags & Protocol245.modeSpecificMutators[mutator]) {
          if (idx < gameSpecificMutators.length) {
            muts.push(gameSpecificMutators[idx]);
          }
        }
...
Protocol245.prototype.masterModeFromCode = function(code) {
	if (code < 0 || code > Protocol245.masterModes.length) {
	  return 'unknown';
	} else {
	  return Protocol245.masterModes[code];
	}
};

Protocol245.prototype.gameModeFromCode = function(code) {
	if (code < 0 || code > Protocol245.gameModes.length) {
	  return 'unknown';
	} else {
	  return Protocol245.gameModes[code];
	}
};
...
if (typeof exports == 'object') {
  ...
  exports.Protocol220 = Protocol220;
  exports.Protocol226 = Protocol226;
  exports.Protocol245 = Protocol245;
}
